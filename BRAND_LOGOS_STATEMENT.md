...
Our official stance on including brand logos in YCloud

## TL;DR

YCloud **does not accept** brand logos, and we do not plan to add them in the future.
...
- **YCloud** learned from these examples — we'd rather focus on making a consistent set of non-brand icons that all work together.
...
- **Don't allow modification** — but we'd have to change them to fit YCloud's style.

This means adding them could:

1. Break copyright or trademark law.
2. Make both you and the YCloud Icons project legally responsible.
...
YCloud is all about **visual consistency**.

Adding brand logos would:

- Break [our own design rules](https://tianjianjun0727.github.io/ycloud-icons/contribute/icon-design-guide#icon-design-principles) for shapes, proportions, and stroke.
...
If the logos are not in YCloud's style, why include them in YCloud at all? Better to use them from a dedicated brand icon source.
...
You can use these alongside YCloud without bloating our core library.

## Final Words

YCloud is an **icon** set, not a **logo** set.

Logos belong in dedicated logo resources.

We're focusing on what YCloud does best: providing a clean, cohesive, and legally safe collection of open-source icons.

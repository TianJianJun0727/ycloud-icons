...
- **业务图标**：位于 `business-icons/`，按 `outlined`、`filled`、`multicolor` 区分颜色模式，文件名使用 kebab-case，通过各包的 `business` 子入口和静态包的 `business-icons/`、`business-font/` 输出。
...
`@ycloud-web/icons-data` 同时提供通用图标节点数据、业务图标 SVG / data URI 数据和插画 SVG / data URI 数据。
...
- **一源多端**：通用图标、业务图标和插画分别从源码目录生成多框架包、静态资源和图标数据。

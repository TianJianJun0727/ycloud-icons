 ...
 * 按分类批量润色图标元数据。
 *
 * 输入：通过 CLI 参数选择分类、批次大小、起止位置和 dry-run 模式。
 * 处理对象：读取指定分类下的 `icons/*.json`，以英文名称、英文标签、英文 use-cases 和分类上下文为语义来源。
 * 主要功能：
 * - 批量生成或改写中文 `name`、`tags`、`use-cases`。
 * - 保留英文元数据作为 source，不用中文反推英文。
 * - 中文标签按自然搜索词去重，不要求与英文标签一一对应。

...
const fetchCompareTags = (oldTag: string) =>
  githubApi(`https://api.github.com/repos/TianJianJun0727/ycloud-icons/compare/${oldTag}...main`);
...
const fetchCommits = async (file) => {
  const commits = await githubApi(
    `https://api.github.com/repos/TianJianJun0727/ycloud-icons/commits?path=${file.filename}`,
  );
  ...
  try {
    ...
    const changedFiles = output.files.filter(
      ({ filename }: { filename: string }) =>
        !filename.match(/docs\/(.*)|(.*)package\.json|tags.json/g),
    );

...
import { data } from './IllustrationCategoryList.data';
import SidebarTitle from './SidebarTitle.vue';

const { page } = useData();
const isEnglish = computed(() => page.value.relativePath?.startsWith?.('en/') ?? false);
const overviewHref = computed(() =>
  isEnglish.value ? '/en/illustration-icons/' : '/illustration-icons/',
);

const headers = computed(() => {
  const pagePath = isEnglish.value ? '/en/illustration-icons/' : '/illustration-icons/';
...
<template>
  <div class="illustration-category-list">
          ...
          <li v-for="{ link, title, iconCount, name } in headers">
            ...
            >
              ...
              <span
                class="icon-count"
                :aria-label="
                  isEnglish ? `${iconCount} illustrations in ${title}` : `${title} 分类下的插画数量`
                "
              >
                {{ iconCount }}
              </span>

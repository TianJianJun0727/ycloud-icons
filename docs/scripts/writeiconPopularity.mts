...
const writePopularityData = async (iconVisitCounts: Record<string, number>) => {
...
if (process.env.YCLOUD_ANALYTICS_TOKEN === undefined) {
  console.warn('YCLOUD_ANALYTICS_TOKEN is not set. Writing default icon popularity data.');
  await writePopularityData(defaultIconNamePopularity);
  process.exit(0);
}

try {
  const response = await fetch(
    'https://analytics.tianjianjun0727.github.io/ycloud-icons/api/v2/query',
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${process.env.YCLOUD_ANALYTICS_TOKEN}`,
      },
      body: JSON.stringify({
        site_id: 'tianjianjun0727.github.io/ycloud-icons',
        metrics: ['visitors'],
  ...
  await writePopularityData(iconVisitCounts);
} catch (error) {

...
const fetchAllReleases = async () => {
  await git.fetch('https://github.com/TianJianJun0727/ycloud-icons.git', '--tags');
...
const comparisons = (await Promise.all(comparisonsPromises)).filter(
  (comparison) => comparison != null && Object.keys(comparison.iconFiles).length > 0,
);

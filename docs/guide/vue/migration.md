...
See what is changed in v1 and how to migrate from v0 to v1 of YCloud for Vue. For all the details about v1 see [Version 1](../version-1.md)

## From `@ycloud-web/icons-vue` to `@ycloud-web/icons-vue`

YCloud Icons package for Vue is moved to `@ycloud-web/icons-vue` in v1.

Mostly a find-and-replace of `@ycloud-web/icons-vue` to `@ycloud-web/icons-vue`. The API is the same, so no other changes are required.

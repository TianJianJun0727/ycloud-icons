...
YCloud Icons 也提供 Web Font 版本。通用图标和业务图标会生成到不同的字体目录，避免 `icon-*` 和 `business-icon-*` 混在同一份 CSS 中。插画不生成字体，请使用 `illustration` 组件入口或 `illustration-icons/<category>/*.svg` 静态资源。

...
- `filled`：将白色填充/描边保留为固定 `#fff`，其他颜色转为 `var(--business-icon-primary-color)`，不按路径顺序判断
...
- `filled` 的 `fill`、`stroke` 只能是 `var(--business-icon-primary-color)`、`#fff` 或 `none`
...
React 业务图标组件底层渲染为内联 `<svg>`。`outlined` 和 `filled` 支持 `size`、`color`、`strokeWidth`；`strokeWidth` 默认不设置，保留源 SVG 的原始描边宽度或无描边状态，仅在显式传入时覆盖。`filled` 的白色细节固定为 `#fff`，不再暴露第二颜色参数。`multicolor` 保留固定色，不暴露 `color` 或 `strokeWidth`，仅支持调整尺寸。
...
静态 SVG 与数据包会保留清洗后的主色 token 和固定白色。使用 `filled` 源时，打包到组件阶段会把主色 token 转为对应框架的 `color` 参数；多色图标始终保留源 SVG 固定色。
